...
def parse_indoor_bike_data(value: bytes) -> Dict[str, float]:
    ...
    # Flag bit meanings (common):
    # 0: more data? / instantaneous speed present (often)
    ...
    # ... (many more)
    #
    # In practice, many trainers include:
    #   speed (uint16, 0.01 km/h), cadence (uint16, 0.5 rpm), power (int16, watts)
    #
    # We'll follow a common field order used by FTMS implementations.

    def need(n: int) -> bool:
        return i + n <= len(value)

    # instantaneous speed (uint16, 0.01 km/h) - often present even if flags vary
    # We'll only read it if flags bit0 is NOT "speed not present". Many devices always include it.
    if need(2):
...
async def main() -> None:
    devices = await WFTNPClient.discover(timeout=2.5)
    if not devices:
        print("No _wahoo-fitness-tnp devices found on LAN")
        return

    dev = next(iter(devices.values()))
    print(f"Found: {dev.name} at {dev.address}:{dev.port}")
    if dev.properties:
        print(f"TXT: {dev.properties}")

    c = WFTNPClient()

    async def on_notify(char_uuid: uuid.UUID, value: bytes) -> None:
        if char_uuid == INDOOR_BIKE_DATA_UUID:
            data = parse_indoor_bike_data(value)
            if data:
                # Keep it short; you can log/store it however you like
                print(f"IndoorBikeData: {data}")

    c.set_notification_callback(on_notify)

    await c.connect(dev.address, dev.port)

    try:
        await c.ftms_init(subscribe_indoor_bike_data=True, subscribe_status=False)
        await c.zwift_like_handshake()

        print("ERG 120 W (safe) ...")
        await c.set_erg_watts(120, max_watts=400)
        await asyncio.sleep(100)

        print("ERG 200 W ...")
        await c.set_erg_watts(200, max_watts=400)
        await asyncio.sleep(100)

    finally:
        # Best-effort: return to easy settings
        try:
            await c.set_erg_watts(80, max_watts=400)
        except Exception:
            pass
        try:
            await c.stop_training()
        except Exception:
            pass
